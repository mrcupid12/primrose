...
jQuery(document).ready(function(){
	...
	jQuery('.changetheme a').click(function(){
		...
		if(jQuery('#addonstyle').length == 0) {
			if(c != 'default') {
				jQuery('head').append('<link id="addonstyle" rel="stylesheet" href="'+jQuery("#url").val()+'statics/css/admin/style.'+c+'.css" type="text/css" />');
				jQuery.cookie("addonstyle", c, { path: '/' });
			}
		} else {
			if(c != 'default') {
				jQuery('#addonstyle').attr('href',jQuery("#url").val()+'statics/css/admin/style.'+c+'.css');
				jQuery.cookie("addonstyle", c, { path: '/' });
			} else {
				jQuery('#addonstyle').remove();	
				jQuery.cookie("addonstyle", null, { path: '/' });
			}
		}
	});
